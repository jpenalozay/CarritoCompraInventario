...
class RevenueController {
    ...
    async getRevenueByCountry(req, res) {
        try {
            ...
            const schema = Joi.object({
                ...
                startDate: Joi.date().iso().default(moment().subtract(7, 'days').format('YYYY-MM-DD')),
                endDate: Joi.date().iso().default(moment().format('YYYY-MM-DD')),
    ...
    async getRevenueSummary(req, res) {
        try {
            const schema = Joi.object({
                startDate: Joi.date().iso().default(moment().subtract(30, 'days').format('YYYY-MM-DD')),
                endDate: Joi.date().iso().default(moment().format('YYYY-MM-DD')),
                useCache: Joi.boolean().default(true)
            });
    ...
    async getAvailableCountries(req, res) {
        try {
            ...
            const result = await require('../database/cassandra').executeQuery(query, [today]);
            ...
            // Cache por 1 hora ya que la lista de países no cambia frecuentemente
